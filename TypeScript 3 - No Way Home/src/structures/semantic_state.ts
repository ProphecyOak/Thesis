...
class SemanticState {
  ...
  private returnValue: TreeNode<any> | undefined;
  ...
  returnNode(val: TreeNode<any>) {
    if (this.parentScope == null)
      throw new Error(`${ERROR_TYPE} Cannot return from global scope.`);
    this.parentScope.returnValue = val;
  }

  getParent(): SemanticState {
    if (this.parentScope) return this.parentScope;
    throw new Error("No higher scope.");
  }
}
